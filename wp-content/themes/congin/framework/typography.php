...
// Start class
if ( ! class_exists( 'Congin_Typography' ) ) {
	class Congin_Typography {
		public function __construct() {
			// Customizer actions
			add_action( 'customize_register', array( 'Congin_Typography' , 'register' ), 40 );

			// Loads Google fonts
			add_action( 'wp_enqueue_scripts', array( 'Congin_Typography', 'load_fonts' ) );
		
			// CSS output
			if ( is_customize_preview() ) {
				add_action( 'customize_preview_init', array( 'Congin_Typography', 'customize_preview_init' ) );
				add_action( 'wp_head', array( 'Congin_Typography', 'live_preview_styles' ), 999 );
			} else {
				add_filter( 'congin_custom_colors_css', array( 'Congin_Typography', 'head_css' ), 999 );
			}
		}
		...
		public static function elements() {
			// Set default font
			$body_default = 'Roboto';
			$heading_default = 'Roboto';

			// Return settings
			$array = apply_filters( 'congin_typography_settings', array(
				'body' => array(
					'label' => esc_html__( 'Body', 'congin' ),
					'target' => 'body',
					'defaults' => array(
				...
				'headings' => array(
					'label' => esc_html__( 'Headings', 'congin' ),
				...
				'featured_subtitle' => array(
					'label' => esc_html__( 'Featured Sub-Title', 'congin' ),
					'target' => '#featured-title .sub-title',
					'exclude' => array( 'font-color' ),
					'active_callback' => 'congin_cac_has_featured_title_heading',
				),
				'breadcrumbs' => array(
					'label' => esc_html__( 'Breadcrumbs', 'congin' ),
					'target' => '#featured-title #breadcrumbs',
					'exclude' => array( 'font-color', 'line-height' ),
					'active_callback' => 'congin_cac_has_featured_title_breadcrumbs',
				),
				'blog_post_title' => array(
					'label' => esc_html__( 'Blog Post Title', 'congin' ),
					'target' => '.hentry .post-title',
					'exclude' => array( 'font-color' ),
				),
				'blog_single_post_title' => array(
					'label' => esc_html__( 'Blog Single Post Title', 'congin' ),
					'target' => '.is-single-post .hentry .post-title',
					'exclude' => array( 'font-color' ),
				),
				'theme_button' => array(
					'label' => esc_html__( 'Buttons', 'congin' ),
					'target' => 'button, input[type="button"], input[type="reset"], input[type="submit"]',
					'exclude' => array( 'font-color', 'line-height' ),
				),
				'theme_pagination' => array(
					'label' => esc_html__( 'Pagination', 'congin' ),
					'target' => '.congin-pagination, .woocommerce-pagination',
					'exclude' => array( 'font-color', 'line-height' ),
				),
				'sidebar_widget_title' => array(
					'label' => esc_html__( 'Sidebar Widget Title', 'congin' ),
					'target' => '#sidebar .widget .widget-title',
				),
				'footer_widget_title' => array(
					'label' => esc_html__( 'Footer Widget Title', 'congin' ),
					'target' => '#footer .widget .widget-title',
					'exclude' => array( 'font-color' ),
				),
				'bottom_nav' => array(
					'label' => esc_html__( 'Bottom Menu', 'congin' ),
					'target' => '#bottom .bottom-bar-copyright a',
					'exclude' => array( 'font-color' ),
				),
				'entry_h1' => array(
					'label' => esc_html__( 'H1', 'congin' ),
					'target' => 'h1',
				),
				'entry_h2' => array(
					'label' => esc_html__( 'H2', 'congin' ),
					'target' => 'h2',
				),
				'entry_h3' => array(
					'label' => esc_html__( 'H3', 'congin' ),
					'target' => 'h3',
				),
				'entry_h4' => array(
					'label' => esc_html__( 'H4', 'congin' ),
					'target' => 'h4',
				),
				'copyright' => array(
		...
		public static function customize_preview_init() {
			wp_enqueue_script( 'congin-typography-customize-preview',
				get_template_directory_uri() .'/framework/customizer/typography-customize.js',
				array( 'customize-preview' ),
				'1.0.0',
				true
			);
			wp_localize_script( 'congin-typography-customize-preview', 'congin', array(
		...
		public static function register ( $wp_customize ) {
			...
			// Add General Panel
			$wp_customize->add_panel( 'congin_typography', array(
				'priority' => 142,
				'capability' => 'edit_theme_options',
				'title' => esc_html__( 'Typography', 'congin' ),
			) );

			// Add General Tab
			$wp_customize->add_section( 'congin_typography_general' , array(
				'title' => esc_html__( 'General', 'congin' ),
				'priority' => 1,
				'panel' => 'congin_typography',
			) );
			...
			$wp_customize->add_control( new congin_Customize_Multicheck_Control( $wp_customize, 'google_font_subsets', array(
				'label' => esc_html__( 'Font Subsets', 'congin' ),
				'section' => 'congin_typography_general',
			...
			foreach( $elements as $element => $array ) {
				...
				if ( $label ) {
					...
					// Define Section
					$wp_customize->add_section( 'congin_typography_'. $element , array(
						'title' => $label,
						'priority' => $count,
						'panel' => 'congin_typography',
					...
					if ( in_array( 'font-family', $attributes ) ) {
						...
						// Add Control
						$wp_customize->add_control( new Congin_Fonts_Dropdown_Custom_Control( $wp_customize, $element .'_typography[font-family]', array(
								'label' => esc_html__( 'Font Family', 'congin' ),
								'section' => 'congin_typography_'. $element,
					...
					if ( in_array( 'font-weight', $attributes ) ) {
						$wp_customize->add_setting( $element .'_typography[font-weight]', array(
							'type' => 'theme_mod',
							'description' => esc_html__( 'Note: Not all Fonts support every font weight style.', 'congin' ),
							'sanitize_callback' => false,
							'transport' => $transport,
						) );
						$wp_customize->add_control( $element .'_typography[font-weight]', array(
							'label' => esc_html__( 'Font Weight', 'congin' ),
							'section' => 'congin_typography_'. $element,
							...
							'choices' => array(
								...
								'100' => esc_html__( 'Extra Light: 100', 'congin' ),
								'200' => esc_html__( 'Light: 200', 'congin' ),
								'300' => esc_html__( 'Book: 300', 'congin' ),
								'400' => esc_html__( 'Normal: 400', 'congin' ),
								'500' => esc_html__( 'Medium: 500', 'congin' ),
								'600' => esc_html__( 'Semibold: 600', 'congin' ),
								'700' => esc_html__( 'Bold: 700', 'congin' ),
								'800' => esc_html__( 'Extra Bold: 800', 'congin' ),
								'900' => esc_html__( 'Black: 900', 'congin' ),
							),
							'description' => esc_html__( 'Important: Not all fonts support every font-weight.', 'congin' ),
						) );
					}
					...
					if ( in_array( 'font-style', $attributes ) ) {
						...
						$wp_customize->add_control( $element .'_typography[font-style]', array(
							'label' => esc_html__( 'Font Style', 'congin' ),
							'section' => 'congin_typography_'. $element,
							...
							'choices' => array(
								'' => esc_html__( 'Default', 'congin' ),
								'normal' => esc_html__( 'Normal', 'congin' ),
								'italic' => esc_html__( 'Italic', 'congin' ),
							),
						) );
					}
					...
					if ( in_array( 'text-transform', $attributes ) ) {
						...
						$wp_customize->add_control( $element .'_typography[text-transform]', array(
							'label' => esc_html__( 'Text Transform', 'congin' ),
							'section' => 'congin_typography_'. $element,
							...
							'choices' => array(
								'' => esc_html__( 'Default', 'congin' ),
								'capitalize' => esc_html__( 'Capitalize', 'congin' ),
								'lowercase' => esc_html__( 'Lowercase', 'congin' ),
								'uppercase' => esc_html__( 'Uppercase', 'congin' ),
							),
						) );
					}
					...
					if ( in_array( 'font-size', $attributes ) ) {
						...
						$wp_customize->add_control( $element .'_typography[font-size]', array(
							'label' => esc_html__( 'Font Size', 'congin' ),
							'section' => 'congin_typography_'. $element,
							'settings' => $element .'_typography[font-size]',
							'priority' => 5,
							'type' => 'text',
							'description' => esc_html__( 'Value in px or em.', 'congin' ),
							'active_callback' => $active_callback,
						) );
					}
					...
					if ( in_array( 'font-color', $attributes ) ) {
						...
						$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, $element .'_typography_color', array(
							'label' => esc_html__( 'Font Color', 'congin' ),
							'section' => 'congin_typography_'. $element,
					...
					if ( in_array( 'line-height', $attributes ) ) {
						...
						$wp_customize->add_control( $element .'_typography[line-height]',
							array(
								'label' => esc_html__( 'Line Height', 'congin' ),
								'section' => 'congin_typography_'. $element,
					...
					if ( in_array( 'letter-spacing', $attributes ) ) {
						...
						$wp_customize->add_control( new WP_Customize_Control( $wp_customize, $element .'_typography_letter_spacing', array(
							'label' => esc_html__( 'Letter Spacing', 'congin' ),
							'section' => 'congin_typography_'. $element,
		...
		public static function loop( $return = 'css' ) {
			...
			foreach( $elements as $element => $array ) {
				...
				$get_mod = congin_get_mod( $element .'_typography' );
		...
		public static function load_fonts() {
			...
			if ( ! empty( $fonts ) && is_array( $fonts ) ) {
				foreach ( $fonts as $font ) {
					congin_enqueue_google_font( $font );
				}
			}
