...
class MAE_Related_Post_Widget extends Widget_Base {
    ...
    public function get_name() {
        return 'mae-related-post';
    }

    // The get_title() method, which again, is a very simple one, you need to return the widget title that will be displayed as the widget label.
    public function get_title() {
        return __( 'MAE - Related Post', 'masterlayer' );
    }
            ...
            if ( $query->have_posts() ) : ?>
                <?php while ( $query->have_posts() ) : $query->the_post(); 
                    ...
                    switch ( $settings['style'] ) {
                        case 'style-1':
                            $post_date = sprintf(
                            '<span class="post-date trapezoid">%1$s</span>', 
                            get_the_date('d M'));
                            break;
                        case 'style-3':
                            $post_date = sprintf(
